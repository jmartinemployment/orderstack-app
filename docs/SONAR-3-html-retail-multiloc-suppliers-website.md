Fix all open SonarCloud HTML accessibility issues in the orderstack-app project for ONLY these feature directories. Do NOT touch any .ts, .scss, or .html files outside these directories:
  ...
  features/retail/ (all subdirs: retail-pos, catalog-management, inventory, reports, vendor-management, purchase-orders, returns, variation-editor, fulfillment, ecommerce)
  features/multi-location/
  features/suppliers/
  features/website/

Approximate issue count: 271 across these directories.
...
For each file: read the full HTML template, identify every SonarCloud violation, and fix all of them in one rewrite. Do not change component logic or styles. Run ng build after all fixes to verify compilation. Use git add -A and commit with message: fix(a11y): resolve HTML accessibility issues in retail, multi-location, suppliers, website — then push.
