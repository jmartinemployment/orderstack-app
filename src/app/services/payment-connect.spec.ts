...
function buildReturnUrl(origin: string, provider: 'stripe' | 'paypal', action: 'complete' | 'refresh'): string {
...
describe('PaymentConnectService — buildReturnUrl', () => {
  it('builds Stripe complete URL', () => {
    expect(buildReturnUrl('https://app.orderstack.com', 'stripe', 'complete'))
      .toBe('https://app.orderstack.com/setup?stripe=complete');
  });

  it('builds Stripe refresh URL', () => {
    expect(buildReturnUrl('https://app.orderstack.com', 'stripe', 'refresh'))
      .toBe('https://app.orderstack.com/setup?stripe=refresh');
  });
