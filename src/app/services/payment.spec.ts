...
function needsExplicitConfirm(processorType: PaymentProcessorType): boolean {
...
describe('PaymentService — isConfigured', () => {
  ...
  it('false when provider is null', () => {
    expect(isConfigured(null, 'stripe')).toBe(false);
  });
...
describe('PaymentService — needsExplicitConfirm', () => {
  it('true for stripe', () => {
    expect(needsExplicitConfirm('stripe')).toBe(true);
  });
...
describe('PaymentService — resolveProvider', () => {
  it('returns stripe for stripe type', () => {
