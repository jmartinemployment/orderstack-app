...
const HARDWARE_CATALOG: HardwareProduct[] = [
  ...
  // --- Card Readers ---
  {
    ...
    whyRecommended: 'Full-featured countertop terminal with built-in display for tips and signatures. Customers see their total and can tip on the reader itself.',
  ...
  },
  {
    ...
    whyRecommended: 'At $59, this is the most affordable way to accept cards. Portable — take it tableside, to events, or on a food truck.',
    processorCompat: 'stripe',
...
export class HardwareGuide {
  ...
  getCompatLabel(compat: string): string {
    if (compat === 'stripe') return 'Works with Stripe';
    if (compat === 'paypal') return 'Works with PayPal';
    if (compat === 'both') return 'Works with Stripe & PayPal';
    return '';
  }
