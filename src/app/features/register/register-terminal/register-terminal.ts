...
export class RegisterTerminal implements OnInit {
  ...
  // Cart state
  private readonly _cartItems = signal<RegisterCartItem[]>([]);
  ...
  addItem(item: MenuItem): void {
    ...
    if (existing) {
      ...
      const cartItem: RegisterCartItem = {
  ...
  joinLoyalty(): void {
    ...
    if (customerInfo) {
      ...
      if (orderId) {
        this.loyaltyService.adjustPoints(orderId, 0, 'Enrolled via register').catch(() => {
