...
describe('IntegrationsPage — filtering logic', () => {
  it('default state (all, empty search) renders all 18 integrations', () => {
    const results = filterIntegrations('all', '');
    expect(results.length).toBe(18);
  });

  it('payments category filters to 3 cards', () => {
    const results = filterIntegrations('payments', '');
    expect(results.length).toBe(3);
    expect(results.map(r => r.id)).toContain('stripe');
  ...
  it('switching back to all with cleared search restores all 18', () => {
    // First filter down
    const filtered = filterIntegrations('payments', 'stripe');
    expect(filtered.length).toBe(1);

    // Then reset
    const restored = filterIntegrations('all', '');
    expect(restored.length).toBe(18);
  });
